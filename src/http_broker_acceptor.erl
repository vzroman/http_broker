...
init(CowboyRequest, #{
  ...
  Request = #request{
    method = parse_method( cowboy_req:method(CowboyRequest1) ),
...
send_to_target({URL, Params}, #request{
  headers = Headers,
  body = Body,
  method = Method
}) ->
  ...
  try
    case httpc:request(Method, Request, HTTPOptions, [{body_format, binary}]) of
      ...
      {ok, {{_, Code, Reason}, ResponseHttpHeaders, ResponseBody}} ->
        {error, Reason, #response{code = Code, headers = ResponseHttpHeaders, body = ResponseBody}};
      {error, Error} ->
        {error, Error, #response{code = 503, headers = [], body = []}}
    end
  catch
    _:HTTPError ->
      {error, HTTPError, #response{code = 500, headers = [], body = []}}
  end.
