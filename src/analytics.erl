...
update_endpoint(Endpoint, EndpointData) ->
    case ets:lookup(request_stats, request_data) of
        [{request_data, Data}] ->
            UpdatedData =
        ...
        [] ->
            % If the table is empty, initialize it with the new data
            InitialData =
                #system_info{endpoints = #{Endpoint => EndpointData},
                             total_queue_count = 0,
                ...
                case maps:find(Target, EndpointData#endpoint_info.targets) of
                    {ok, TargetInfo} ->
                        ...
                        UpdatedTargets =
                            maps:update(Target,
                                        UpdatedTargetInfo,
                                        EndpointData#endpoint_info.targets),
                        UpdatedEndpointData = EndpointData#endpoint_info{targets = UpdatedTargets},
                        UpdatedEndpoints =
                            maps:update(Endpoint, UpdatedEndpointData, Data#system_info.endpoints),
